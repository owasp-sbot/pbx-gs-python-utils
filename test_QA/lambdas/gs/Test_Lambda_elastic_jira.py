...
class test_lambda_elastic_jira(unittest.TestCase):
    def setUp(self):
        ...
        self.jira_issues = Lambdas('gs.elastic_jira')                           # no need to include dependencies in the source code now (3 secs per update)
    ...
    def test_update_invoke(self):
        key = 'RISK-424'
        issue = self.jira_issues.update_with_src().invoke({"params": [key], "channel": 'DDKUZTK6X'})
        Dev.pprint(issue)

    def test_update_invoke__link_links(self):
        id = 'RISK-1'
        result = self.jira_issues.update_with_src().invoke({"params": ["issue-links", id], "channel": 'GDL2EC3EE'})
        Dev.pprint(result)
    ...
    def test_update_invoke__link_share(self):
        ...
        result = self.jira_issues.update_with_src().invoke({"params": [ 'link_shared', '[{"url":"https://jira.photobox.com/browse/SEC-1234"}]'], "channel": 'GDL2EC3EE'})

        Dev.pprint(result)

    def test_update_invoke__test_cmd(self):
        result = self.jira_issues.update_with_src().invoke({"params": [ 'test'], "channel": 'GDL2EC3EE'})
        Dev.pprint(result)



    def test_invoke_bad_cmd(self):
        result = self.jira_issues.update_with_src().invoke({"params": ["aaaa"], 'user' : 'abc'})
    ...
    def test_invoke__no_data_for_issue(self):
        issue_id = 'GSCS-24'
        result = self.jira_issues.update_with_src().invoke({"params": ['issue',issue_id], 'team_id':'T7F3AUXGV', 'channel':'DDKUZTK6X' })
        Dev.pprint(result)