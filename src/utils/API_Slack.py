...
class API_Slack:
    def __init__(self, channel = 'GDL2EC3EE'):      # 'gs-bot-tests'
        self.bot_token = Secrets('slack-gs-bot').value()
    ...
    def users(self):
        #return self.slack.api_call("users.list")
        users = {}
        for user in self.slack.api_call("users.list")['members']:
